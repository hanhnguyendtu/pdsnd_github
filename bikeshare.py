...
def get_filters():
    ...
    while True:
        city = input("Please input the city you desire. You can choose from Chicago, New York City, and Washington: ").lower()
        if city in ['chicago', 'new york city', 'washington']:
            break
        else:
            print("Invalid input. Please only input a city from Chicago, New York City, or Washington")

    # TO DO: get user input for month (all, january, february, ... , june)
    while True:
        month = input("What month would you like? You can choose from January, February, March, April, May, June, or select all: ").lower()
        if month in ['january', 'february', 'march', 'april', 'may', 'june', 'all']:
            break
        else:
            print("Invalid input. Please input a month from January to June, or choose all.")

    # TO DO: get user input for day of week (all, monday, tuesday, ... sunday)
    while True:
        day = input("What day of the week would you like? You can choose Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday, or select all: ").lower()
        if day in ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday', 'all']:
            break
        else:
            print("Invalid input. Please input a day from Monday to Sunday, or choose all: ")
...
def user_stats(df):
    ...
    if 'Birth Year' in df.columns:
        print("\nOldest year of birth: ", int(df['Birth Year'].min()))
        print("Most recent year of birth: ", int(df['Birth Year'].max()))
        print("Most common year of birth: ", int(df['Birth Year'].mode()[0]))
    else:
        print("\nBirth year information is unavailable.")
